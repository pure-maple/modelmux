"""TUI configuration panel for modelmux.

Launch with: modelmux config [--scope user|project]
Requires: pip install modelmux[tui]  (or uvx --with textual modelmux config)
"""
...
class ConfigApp(App):
    """modelmux TUI configuration panel."""

    TITLE = "modelmux config"
    ...
    def __init__(self, scope: str = "user"):
        ...
        self._config_dir = (
            Path.cwd() / ".modelmux"
            if scope == "project"
            else Path.home() / ".config" / "modelmux"
        )
